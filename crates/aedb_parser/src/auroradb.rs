...
fn build_checks(
    ...
    let mut def_net_names = binary_strings.clone();
    def_net_names.extend(def.domain.layout_nets.iter().map(|net| net.name.clone()));
    ...
    vec![
        ...
        membership_check(
            ...
            &def_net_names,
            "AuroraDB electrical net names should be present in DEF LayoutNet records or binary string records; synthetic NONET is generated by the exporter.",
            "fail",
        ),
        membership_check(
...
fn is_synthetic_no_net(name: &str) -> bool {
    matches!(
        ...
        "NONET" | "NO_NET" | "NO-NET" | "<POWER/GROUND>"
    )
}
...
mod tests {
    ...
    fn compares_public_def_cases_to_standard_auroradb_names() {
        ...
        for (def_name, auroradb_name, metal_layers, nets, vias, components, parts) in cases {
            let def_path = fixture(def_name);
            let auroradb_path = fixture(auroradb_name);
            if !def_path.exists() || !auroradb_path.exists() {
                continue;
            }
            let comparison = compare_def_to_auroradb(&def_path, &auroradb_path).unwrap();
            assert_eq!(
