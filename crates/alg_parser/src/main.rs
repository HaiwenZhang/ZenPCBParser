use aurora_alg_native::{build_payload, BuildPayloadOptions, BACKEND_CLI, RUST_PARSER_VERSION};
use std::env;
use std::fs;
...
fn parse_args(args: Vec<String>) -> Result<CliOptions, String> {
    ...
    let mut schema_version = "0.2.0".to_string();
    let mut index = 0;
...
fn help_text() -> String {
    "Usage: alg_parser <board.alg> [--summary-only] [-o OUTPUT]\n\
     \n\
     Options:\n\
       --summary-only           Count extracta sections and skip detail arrays.\n\
       ...
       --parser-version VALUE   ALG parser version injected by Aurora Translator.\n\
       --schema-version VALUE   ALG JSON schema version injected by Aurora Translator."
        .to_string()
}
