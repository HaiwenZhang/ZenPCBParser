...
- AEDB DEF binary parser 更新到 `0.13.0`，独立的 `AEDBDefBinaryLayout` JSON schema 更新到 `0.13.0`；parser 现在会重组跨行 `SLayer(...)` text record，读取 `.def` material 的 conductivity / permittivity / dielectric loss tangent，从原生 `Outline` polygon record 恢复真实圆角 board outline，并保留 text padstack `hle(...)`、`ant(...)`、`thm(...)` 的 shape/尺寸/offset/rotation 字段。默认 PyEDB `AEDBLayout` parser/schema 仍保持 `0.4.56` / `0.5.0`。
- AEDB DEF binary -> SemanticBoard 更新到 `0.7.26`：component placement rotation 现在从 component pin padstack instance 的 source rotation 恢复，AuroraDB target 对 AEDB bottom component 使用 bottom-side `flipY` 约定；`DemoCase_LPDDR4.def` 中 Bottom `C131/C136/C137/C130/C363` 和 Top `R111/C11/R57` 的旋转已对齐 Standard。AuroraDB target 也会按 `CCW Y` 输出规范归一 native polygon / void 点序和圆弧方向，AEDB DEF binary adapter 把 trailing closing arc 纳入 native void coverage；Bottom/GND 右上大圆形 void 与 `VIA8D16` margin-only clearance void 已补齐，该大 void 的直接子 hole 数与 Standard 对齐为 `15/15`。Semantic JSON schema 保持 `0.7.2`，AuroraDB JSON schema 不变。
- AEDB DEF binary -> SemanticBoard 更新到 `0.7.25`：无 ANF sidecar 时会标记 `metadata.source_step=def-binary`，AuroraDB layout/parts/stackup 使用 `mils` 输出单位，同时从米单位 semantic 坐标正确换算；`DemoCase_LPDDR4.def` 对照标准 AuroraDB 后达到 `units=mils`、`components=293`、`parts=58`、`nets=335`、`via_templates=7`、`net_geometries=9785`、`shapes=61`，board outline、stackup 厚度/材料、`C200-109T` 的 `RectCutCorner` barrel 和 `ViaList` ID/order 与标准一致，layer geometry 类型分布对齐为 `Line=7998`、`Polygon=33`、`PolygonHole=39`。source-backed clearance holes 现在覆盖 through-hole/unnamed padstack 的 `84.04/80.04/70.04/51.04mil` 孔；nested holes 当前为 `1733`，Standard 为 `1742`，剩余差异主要是 routing via 的 `24.04mil` / `26.04mil` 细分。Semantic JSON schema 保持 `0.7.2`，AuroraDB JSON schema 不变。
- AEDB DEF binary parser 更新到 `0.11.0`，独立的 `AEDBDefBinaryLayout` JSON schema 更新到 `0.11.0`；`domain.binary_polygon_records[].net_index/net_name` 现在会从同一二进制 primitive 流的 path net 分组恢复，并同步传播给 void polygon。三套无 ANF `.def` 样本的 native polygon owner 均恢复到真实 layout net：Demo/Zynq 为 `GND`，SW 为 `GND_POWER`。
- AEDB DEF binary -> SemanticBoard 现在在 `vias[].geometry` 和 `via_templates[].geometry` 中记录从 padstack-instance definition layer range 恢复的 via taxonomy，包括 `via_type`（`through` / `blind` / `buried` / `single_layer` / `unknown`）、首末层、层序号、跨层数和 `spans_full_stack`；Semantic parser 更新到 `0.7.22`，Semantic JSON schema 保持 `0.7.2`。
- AuroraDB exporter 现在使用 AEDB DEF binary 的 `via_usage` taxonomy 区分 routing via 与 component pin padstack instance；`layout.db` 的 `NetVias` 只直接写出 `routing_via`，component pin 继续走 pin/pad 和必要的 component-pad via 路径，避免把 SMD/Pin padstack 实例误写成真实布线 via。三套无 ANF 样本重新生成后 `NetVias.Via` 行数为 `1156 / 1388 / 1197`，AuroraDB JSON schema 不变。
- AEDB DEF binary parser 更新到 `0.10.0`，独立的 `AEDBDefBinaryLayout` JSON schema 更新到 `0.10.0`；`domain.layout_nets` 输出 `.def` 二进制 net table，`domain.binary_path_records` 输出 path record 的几何 ID、net/layer owner、宽度和点列，`domain.binary_padstack_instance_records` 输出 EDB `PadstackInstance` 字段并新增 `drill_diameter`，`domain.padstacks[].layer_pads[]` 保留 text padstack `pad(shp, Szs, X/Y/R)` 尺寸字段，`domain.padstack_instance_definitions[]` 输出 `raw_definition_index -> padstack_id` 和真实 layer range 映射，`domain.binary_polygon_records` 输出原生二进制 polygon/void 的 geometry id、parent id、layer、点列和 arc-height marker。默认 PyEDB `AEDBLayout` parser/schema 仍保持 `0.4.56` / `0.5.0`。
- 逆向验证 `examples/edb_cases/{DemoCase_LPDDR4,SW_ARM_1029,Zynq_Phoenix_Pro}.def`：`.def` 是带 text DSL record 与 binary record 的 AEDB 存储流，不是 ANF 的简单二进制封装；三套同版本样本的二进制 path 恢复为 `1965 / 2833 / 2208` 条，`PadstackInstance` 恢复为 `2843 / 3531 / 2709` 条，原生 polygon/void record 恢复为 `840 / 403 / 744` 条，padstack-instance definition 映射恢复为 `80 / 112 / 52` 条。
- `convert --from aedb --aedb-backend def-binary --to auroradb` 现在接入 `AEDBDefBinaryLayout -> SemanticBoard -> AuroraDB` 端到端路径。三套同版本 `.def` 样本均可在删除 ANF sidecar 后生成标准 AuroraDB 目录，包含 `layout.db`、`parts.db`、`stackup.dat`、`stackup.json` 和 `layers/*.lyr`；native `binary_polygon_records` 输出 layer `Polygon` / `PolygonHole` / `Holes`，并从大平面 bbox 推断 board outline。pad template 会通过原生 padstack-instance definition 表恢复 text padstack 的 circle/rectangle/square/oval shape；在 ANF sidecar 存在时，adapter 仍优先用 ANF 的 `via(...)` / `Padstacks` / `PolygonWithVoids` / outline graphics 补齐 padstack 名称、层跨度、polygon pad shape、polygon holes 和真实 board outline。Semantic parser 更新到 `0.7.21`，Semantic/AuroraDB JSON schema 不变。
- 修正 Altium -> SemanticBoard 层栈选择：adapter 现在从 PcbDoc 的 `LAYERnNEXT` / `LAYERnPREV` 链路恢复实际 stackup，并把 `LAYER39`..`LAYER54` internal plane 纳入金属层；不再把 `LAYER1`..`LAYER32NAME` 的未启用中间层槽位全部导出为 AuroraDB 金属层。Semantic parser 更新到 `0.7.15`，Semantic JSON schema 保持 `0.7.2`，Altium source JSON schema 不变。
- Altium adapter 现在会跳过没有任何 pad 的 source component，避免将 logo / mechanical-only component 写成没有可渲染 `PartPad` 的 AuroraDB footprint；Altium source JSON schema 不变。
- 验证 `examples/altium_cases/{6735,VR,XC7Z020CLG400-AD9,demo1,demo2}` 的 Altium / ODB++ 成对转换输出在 `examples/altium_cases/_compare_fixed/`：AuroraDB `stackup.json` 金属层数分别对齐为 `6/6/14/33/4`；ODB++ 的 soldermask、dielectric、drill、component 等非金属 matrix rows 仍计入 ODB++ Semantic 总层数，但不计入 AuroraDB `Metal` 层。
- 修正 AuroraDB parts exporter 的 footprint 精确引用问题：同一 footprint 在 source component 中出现大小写差异时，`Part.FootPrintSymbols` 现在会引用规范化后的实际 `FootPrintSymbol.SymbolID`，避免大小写敏感渲染器报 `Footprint symbol does not exist`。AuroraDB JSON schema 不变。
- 验证 Altium 生成的 `examples/altium_cases/_compare_fixed/{6735,VR,XC7Z020CLG400-AD9,demo1,demo2}/altium_auroradb`：component part 引用、part footprint 引用、空 footprint 引用、footprint metal layer 和 layer file 关系校验均为 `OK`。
- 修正 `examples/altium_cases/DemoCase_LPDDR4` 的 Altium / ODB++ 到 AuroraDB parts 对齐：Altium adapter 会从 `Inside Board Components` class 和 comment text 恢复 refdes / part name，并跳过无名 no-net component pad；ODB++ adapter 会归并 `_1_2` 这类重复 package 后缀、优先使用 package pin name、过滤 `empty-*` package pin，并规范化 part name 中的小数尺寸单位；AuroraDB parts exporter 对 ODB++ 有 package pad 的 footprint 改为优先导出 package pad。Semantic parser 更新到 `0.7.16`，Semantic JSON schema 保持 `0.7.2`。
- 验证 `examples/altium_cases/_debug/altium_auroradb_final2` 和 `examples/altium_cases/_debug/odbpp_auroradb_final3` 对照 `examples/altium_cases/DemoCase_LPDDR4_Standard`：`parts=58`、`footprints=27`、`components=293`，component part 引用、part footprint 引用、空 footprint、footprint pad IDs 和 Part PinList 差异均为 `0`；ODB++ layout 仍保留源单位 `mm`，几何展开数量与标准的 mils 参考输出不同。
- 修正 `examples/altium_cases/DemoCase_LPDDR4` 的 Altium 金属层几何异常放大：Altium adapter 现在把 PcbDoc 坐标归一到 AuroraDB/ODB++ 使用的 Y 轴方向，并过滤 `2147483647` 无效坐标哨兵；Altium parser `0.1.1` 现在按 Vias6 的 8.8 fixed-point 编码还原 per-layer via diameter，避免把 18/16mil via pad 写成 4608/4096mil 圆形；adapter 也会兜底归一旧 source JSON 中已放大的 `diameter_by_layer`。重新生成的 `examples/altium_cases/DemoCase_LPDDR4_Altium_AuroraDB` 不再包含 `214748.3647` 顶点或 `Circle 0 0 4608` / `Circle 0 0 4096` 形状，layer 坐标范围回到约 `20..1980 mil`。Semantic parser 更新到 `0.7.17`，Semantic JSON schema 保持 `0.7.2`，Altium JSON schema 保持 `0.1.0`。
- 修正 Altium polygon-with-holes、圆角几何和 solder-side marker pad 导出：Altium parser `0.1.2` 现在按 KiCad C++ importer 的分工同时解析 `ShapeBasedRegions6` 与 legacy `Regions6`，把 shape-based outline 顶点、double 坐标 holes 和 `Regions6` filled polygon 分开处理；Altium adapter 会让 filled region 从引用的 polygon pour 继承真实 net、避免 shape-based / legacy region 重复导出，并把 `Top Solder` / `Bottom Solder` pad 映射到对应 metal side。重新生成的 `examples/altium_cases/XC7Z020CLG400-AD9_Altium_AuroraDB` 不再有 missing pad layer diagnostics，`GND02` / `PWR13` 等带 `Regions6` fill 的内层会写出 `PolygonHole` / `Holes`。Semantic parser 更新到 `0.7.18`，Semantic JSON schema 保持 `0.7.2`，Altium JSON schema 保持 `0.1.0`。
- 修正 `ODB++ -> SemanticBoard -> AuroraDB` 的 Mentor Xpedition component 对齐：component pin 和 package-pad fallback 现在按 matrix 中最外侧 signal / plane 层映射 top/bottom side，不再写出字面量 `TOP` / `BOTTOM`；component placement 会叠加 Xpedition `REFLOC` 偏移，AuroraDB exporter 对 ODB++ bottom component 使用 `flipY` 和取反旋转，匹配标准 AuroraDB 的 bottom placement 约定。Semantic parser 更新到 `0.7.15`，Semantic JSON schema 保持 `0.7.2`。
- 修正 `ODB++ -> SemanticBoard -> AuroraDB` 的 Xpedition via 解析：matrix `DRILL` rows（如 `d_1_2`）现在会生成 blind/buried via；无 net 的 drill feature 归入 `NoNet`，`$NONE$;;ID=...` 形式的 no-net 名称会合并为 `NoNet`；via template refinement 优先匹配非 component 的 via land pad，并忽略圆形 pad 的无意义旋转，避免同一 via padstack 被拆成大量模板。
- 修正 AuroraDB target 的单位策略：layout、parts 和 stackup 现在共用同一套源单位导出规则，能表达的 SemanticBoard 源单位会保留到 AuroraDB 输出中，不再只在 layout 或 parts 单侧转换为 `mils`；同时 ODB++ footprint variant 会把 `PartPad` 的局部位置、旋转和 flip 纳入签名，避免不同 `REFLOC` 元件共用同一 package footprint 后 pin1/pin2 局部位置相反。
- BRD -> SemanticBoard 不再把 mils 源坐标先规范化成 mm；BRD header 声明 `mils` 时，Semantic 和 AuroraDB target 会继续使用 `mil` 作为源单位。无法可靠识别单位的 BRD 仍回退到 mm。
- 修正 Mentor Xpedition package 的 ODB++ AuroraDB parts 输出：当 footprint 由真实 component pad 反推时，exporter 不再把 package body outline 写成 footprint metal-layer geometry。这样会移除 `parts.db` 中额外偏移的 `GeometryList` 矩形，同时保留真实 component pad 推导出的 `PadTemplate` 和 `PartPad` 尺寸。
- 验证 `examples/odbpp_cases/LD20-28567_B_MAIN_BOARD.tgz` 对照 `examples/odbpp_cases/LD20-28567_B_MAIN_BOARD_Standard`：Semantic 输出为 `via_templates=15`、`vias=65766`、`diagnostics=0`，AuroraDB 回读为 `vias=15`、`diagnostics=0`；抽样 component 的 `REFLOC` 后坐标、bottom rotation 和 flip 标志与标准输出一致。验证 `<ODBPP_CASES>/Mentor-Xpedition-batch` 15 个 `.tgz` 样本：ODB++ source diagnostics 均为 `0`，全部可转换到 AuroraDB 并回读，AuroraDB diagnostics 均为 `0`，Semantic diagnostics 均为 `0`。
...
- AEDB DEF binary parser is now `0.13.0`, and the separate `AEDBDefBinaryLayout` JSON schema is now `0.13.0`; the parser reassembles multi-line `SLayer(...)` text records, reads `.def` material conductivity / permittivity / dielectric loss tangent, recovers the real rounded board outline from native `Outline` polygon records, and preserves text-padstack `hle(...)`, `ant(...)`, and `thm(...)` shape/size/offset/rotation fields. The default PyEDB `AEDBLayout` parser/schema remain `0.4.56` / `0.5.0`.
- AEDB DEF binary -> SemanticBoard is now `0.7.26`: component placement rotation is recovered from component-pin padstack-instance source rotation, and the AuroraDB target uses the bottom-side `flipY` convention for AEDB bottom components; DemoCase Bottom `C131/C136/C137/C130/C363` and Top `R111/C11/R57` rotations now align with Standard. The AuroraDB target also normalizes native polygon / void point order and arc direction to the emitted `CCW Y` convention, and the AEDB DEF binary adapter includes trailing closing arcs in native-void coverage; the Bottom/GND upper-right large circular void plus `VIA8D16` margin-only clearance voids are present, and direct child holes for that large void align with Standard at `15/15`. Semantic JSON schema remains `0.7.2`, and AuroraDB JSON schema is unchanged.
- AEDB DEF binary -> SemanticBoard is now `0.7.25`: when no ANF sidecar is present it marks `metadata.source_step=def-binary`, and AuroraDB layout/parts/stackup export in `mils` while converting the meter-based semantic coordinates correctly. Against the standard AuroraDB for `DemoCase_LPDDR4.def`, the direct DEF-binary output reaches `units=mils`, `components=293`, `parts=58`, `nets=335`, `via_templates=7`, `net_geometries=9785`, and `shapes=61`; the board outline, stackup thickness/material values, `C200-109T` `RectCutCorner` barrel, and `ViaList` ID/order match the standard, and layer geometry types align at `Line=7998`, `Polygon=33`, and `PolygonHole=39`. Source-backed clearance holes now cover through-hole/unnamed padstack `84.04/80.04/70.04/51.04mil` holes; nested holes are currently `1733` versus Standard `1742`, with the remaining difference mainly in routing-via `24.04mil` / `26.04mil` subdivision. Semantic JSON schema remains `0.7.2`, and AuroraDB JSON schema is unchanged.
- AEDB DEF binary parser is now `0.11.0`, and the separate `AEDBDefBinaryLayout` JSON schema is now `0.11.0`; `domain.binary_polygon_records[].net_index/net_name` is recovered from path net grouping in the same binary primitive stream and propagated to void polygons. The three no-ANF `.def` samples now recover native polygon owners to the real layout net: Demo/Zynq to `GND`, and SW to `GND_POWER`.
- AEDB DEF binary -> SemanticBoard now records via taxonomy in `vias[].geometry` and `via_templates[].geometry` from padstack-instance definition layer ranges, including `via_type` (`through` / `blind` / `buried` / `single_layer` / `unknown`), start/stop layers, layer indices, span count, and `spans_full_stack`. Semantic parser is now `0.7.22`; Semantic JSON schema remains `0.7.2`.
- The AuroraDB exporter now uses AEDB DEF binary `via_usage` taxonomy to distinguish routing vias from component-pin padstack instances; `layout.db` `NetVias` directly emits only `routing_via`, while component pins remain represented through pin/pad and required component-pad via paths. This avoids writing SMD/pin padstack instances as real routing vias. Regenerated no-ANF sample `NetVias.Via` line counts are `1156 / 1388 / 1197`; AuroraDB JSON schema is unchanged.
- AEDB DEF binary parser is now `0.10.0`, and the separate `AEDBDefBinaryLayout` JSON schema is now `0.10.0`; `domain.layout_nets` emits the `.def` binary net table, `domain.binary_path_records` emits path geometry ids, net/layer ownership, widths, and point lists, `domain.binary_padstack_instance_records` emits EDB `PadstackInstance` fields plus `drill_diameter`, `domain.padstacks[].layer_pads[]` preserves text padstack `pad(shp, Szs, X/Y/R)` dimensions, `domain.padstack_instance_definitions[]` emits `raw_definition_index -> padstack_id` and real layer-range mappings, and `domain.binary_polygon_records` emits native binary polygon/void geometry ids, parent ids, layers, point lists, and arc-height markers. The default PyEDB `AEDBLayout` parser/schema remain `0.4.56` / `0.5.0`.
- Reverse-engineered `examples/edb_cases/{DemoCase_LPDDR4,SW_ARM_1029,Zynq_Phoenix_Pro}.def`: `.def` is an AEDB storage stream with text DSL records and binary records, not a simple binary packing of ANF; binary paths recover as `1965 / 2833 / 2208` records, `PadstackInstance` records recover as `2843 / 3531 / 2709` records, native polygon/void records recover as `840 / 403 / 744` records, and padstack-instance definition mappings recover as `80 / 112 / 52` records.
- `convert --from aedb --aedb-backend def-binary --to auroradb` now runs the end-to-end `AEDBDefBinaryLayout -> SemanticBoard -> AuroraDB` path. The three same-version `.def` samples generate standard AuroraDB directories after ANF sidecars are removed, with `layout.db`, `parts.db`, `stackup.dat`, `stackup.json`, and `layers/*.lyr`; native `binary_polygon_records` emit layer `Polygon` / `PolygonHole` / `Holes`, and the board outline is inferred from large-plane bboxes. Pad templates recover text-padstack circle/rectangle/square/oval shapes through the native padstack-instance definition table. When an ANF sidecar is present, the adapter still prefers ANF `via(...)`, `Padstacks`, `PolygonWithVoids`, and outline graphics for padstack names, layer spans, polygon pad shapes, polygon holes, and the real board outline. Semantic parser is now `0.7.21`; Semantic/AuroraDB JSON schemas are unchanged.
- Fixed Altium -> SemanticBoard stackup selection: the adapter now recovers the actual stackup from PcbDoc `LAYERnNEXT` / `LAYERnPREV` links and includes `LAYER39`..`LAYER54` internal planes as metal layers, instead of exporting every inactive `LAYER1`..`LAYER32NAME` copper slot as an AuroraDB metal layer. Semantic parser is now `0.7.15`; Semantic JSON schema remains `0.7.2`, and the Altium source JSON schema is unchanged.
- The Altium adapter now skips source components with no pads, avoiding logo / mechanical-only components being written as AuroraDB footprints with no renderable `PartPad`; the Altium source JSON schema is unchanged.
- Verification with paired Altium / ODB++ conversions for `examples/altium_cases/{6735,VR,XC7Z020CLG400-AD9,demo1,demo2}` under `examples/altium_cases/_compare_fixed/`: AuroraDB `stackup.json` metal-layer counts align at `6/6/14/33/4`; ODB++ non-metal matrix rows such as soldermask, dielectric, drill, and component layers still count toward the ODB++ Semantic total layer count, but not toward AuroraDB `Metal` layers.
- Fixed exact footprint references in the AuroraDB parts exporter: when source components use different casing for the same footprint, `Part.FootPrintSymbols` now references the canonical emitted `FootPrintSymbol.SymbolID`, avoiding `Footprint symbol does not exist` in case-sensitive renderers. AuroraDB JSON schema is unchanged.
- Verification with Altium-generated `examples/altium_cases/_compare_fixed/{6735,VR,XC7Z020CLG400-AD9,demo1,demo2}/altium_auroradb`: component part references, part footprint references, blank footprint references, footprint metal layers, and layer-file relationships all validate as `OK`.
- Fixed Altium / ODB++ to AuroraDB parts alignment for `examples/altium_cases/DemoCase_LPDDR4`: the Altium adapter recovers refdes / part names from `Inside Board Components` classes and comment text, and skips unnamed no-net component pads; the ODB++ adapter canonicalizes duplicate package suffixes such as `_1_2`, prefers package pin names, filters `empty-*` package pins, and normalizes decimal unit fragments in part names; the AuroraDB parts exporter now prefers package pads for ODB++ footprints that have package-pad geometry. Semantic parser is now `0.7.16`; Semantic JSON schema remains `0.7.2`.
- Verification against `examples/altium_cases/DemoCase_LPDDR4_Standard` with `examples/altium_cases/_debug/altium_auroradb_final2` and `examples/altium_cases/_debug/odbpp_auroradb_final3`: `parts=58`, `footprints=27`, and `components=293`; component part references, part footprint references, empty footprints, footprint pad IDs, and Part PinList differences are all `0`. ODB++ layout still preserves the source unit `mm`, so geometry expansion counts differ from the mils reference output.
- Fixed oversized Altium metal-layer geometry for `examples/altium_cases/DemoCase_LPDDR4`: the Altium adapter now normalizes PcbDoc coordinates to the Y-axis direction used by AuroraDB/ODB++ and filters invalid `2147483647` coordinate sentinels; Altium parser `0.1.1` now decodes Vias6 per-layer via diameters as 8.8 fixed-point values, avoiding 18/16mil via pads being emitted as 4608/4096mil circles; the adapter also guards older source JSON with already-scaled `diameter_by_layer` values. Regenerated `examples/altium_cases/DemoCase_LPDDR4_Altium_AuroraDB` no longer contains `214748.3647` vertices or `Circle 0 0 4608` / `Circle 0 0 4096` shapes, and layer coordinate ranges return to about `20..1980 mil`. Semantic parser is now `0.7.17`; Semantic JSON schema remains `0.7.2`, and Altium JSON schema remains `0.1.0`.
- Fixed Altium polygon-with-holes, rounded geometry, and solder-side marker pad export: Altium parser `0.1.2` now follows the KiCad C++ importer split by parsing both `ShapeBasedRegions6` and legacy `Regions6`, handling shape-based outline vertices, double-coordinate holes, and `Regions6` filled polygons separately; the Altium adapter lets filled regions inherit the real net from the referenced polygon pour, avoids duplicate shape-based / legacy region export, and maps `Top Solder` / `Bottom Solder` pads to the corresponding metal side. Regenerated `examples/altium_cases/XC7Z020CLG400-AD9_Altium_AuroraDB` no longer has missing pad layer diagnostics, and inner layers with `Regions6` fills such as `GND02` / `PWR13` emit `PolygonHole` / `Holes`. Semantic parser is now `0.7.18`; Semantic JSON schema remains `0.7.2`, and Altium JSON schema remains `0.1.0`.
- Fixed Mentor Xpedition component alignment for `ODB++ -> SemanticBoard -> AuroraDB`: component-pin and package-pad fallback now map top/bottom side to the outermost signal / plane layers from the matrix instead of emitting literal `TOP` / `BOTTOM`; component placement applies the Xpedition `REFLOC` offset, and the AuroraDB exporter writes ODB++ bottom components with `flipY` and mirrored rotation to match standard AuroraDB bottom-placement conventions. Semantic parser is now `0.7.15`; Semantic JSON schema remains `0.7.2`.
- Fixed Xpedition via parsing for `ODB++ -> SemanticBoard -> AuroraDB`: matrix `DRILL` rows such as `d_1_2` now emit blind/buried vias; netless drill features are assigned to `NoNet`, `$NONE$;;ID=...` no-net names collapse to `NoNet`, and via-template refinement prefers non-component via land pads while ignoring meaningless circle-pad rotation so one padstack is not split into many templates.
- Fixed AuroraDB target unit policy: layout, parts, and stackup now share the same source-unit export rule, preserving expressible SemanticBoard source units in AuroraDB output instead of converting only one side of layout or parts to `mils`. ODB++ footprint variants also include `PartPad` local location, rotation, and flip in their signature so components with different `REFLOC` origins do not share a package footprint with reversed pin1/pin2 local positions.
- BRD -> SemanticBoard no longer normalizes mils source coordinates to mm first; when the BRD header declares `mils`, Semantic and the AuroraDB target keep `mil` as the source unit. BRD files with units that cannot be identified reliably still fall back to mm.
- Fixed ODB++ AuroraDB parts output for Mentor Xpedition packages: when component pads drive the footprint, the exporter no longer emits package body outlines as footprint metal-layer geometry. This removes extra offset `GeometryList` rectangles from `parts.db` while preserving `PadTemplate` and `PartPad` sizes from the real component pads.
- Verified `examples/odbpp_cases/LD20-28567_B_MAIN_BOARD.tgz` against `examples/odbpp_cases/LD20-28567_B_MAIN_BOARD_Standard`: Semantic output reports `via_templates=15`, `vias=65766`, and `diagnostics=0`; AuroraDB readback reports `vias=15` and `diagnostics=0`; sampled component `REFLOC`-adjusted coordinates, bottom rotation, and flip flags match the standard output. Verified 15 `.tgz` samples from `<ODBPP_CASES>/Mentor-Xpedition-batch`: all ODB++ source diagnostics are `0`, all samples convert to AuroraDB and read back with AuroraDB diagnostics `0`, and all Semantic diagnostics are `0`.
