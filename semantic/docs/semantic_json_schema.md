...
uv run python .\main.py semantic from-json alg .\out\board.alg.json
uv run python .\main.py semantic from-json alg .\out\board.alg.json -o .\out --semantic-output alg.semantic.json
uv run python .\main.py semantic from-json altium .\out\board.altium.json
uv run python .\main.py semantic from-json altium .\out\board.altium.json -o .\out --semantic-output altium.semantic.json
...
uv run python .\main.py semantic from-source alg <path-to-extracta.alg>
uv run python .\main.py semantic from-source alg <path-to-extracta.alg> -o .\out --semantic-output alg.semantic.json
uv run python .\main.py semantic from-source altium <path-to-board.PcbDoc>
uv run python .\main.py semantic from-source altium <path-to-board.PcbDoc> -o .\out --semantic-output altium.semantic.json
...
对于 ODB++、ALG 和 Altium 源路径，semantic CLI 默认优先使用对应 PyO3 native 模块；传入 `--rust-binary` 时会强制走对应 Rust parser CLI。
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `source_format` | string | `aedb`、`auroradb`、`odbpp`、`brd`、`alg` 或 `altium`。 |
...
- Current Semantic parser version: `0.7.10`
...
For ODB++, ALG, and Altium source paths, the semantic CLI prefers the corresponding PyO3 native module by default; passing `--rust-binary` forces the corresponding Rust parser CLI.
...
| Field | Type | Description |
| --- | --- | --- |
| `source_format` | string | `aedb`, `auroradb`, `odbpp`, `brd`, `alg`, or `altium`. |
