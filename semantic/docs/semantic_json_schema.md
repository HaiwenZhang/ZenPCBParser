...
- 当前项目版本：`1.0.43`
- 当前 Semantic parser 版本：`0.7.5`
- 当前 Semantic JSON schema 版本：`0.7.1`
...
对于 ODB++ 和 ALG 源路径，semantic CLI 默认优先使用对应 PyO3 native 模块；传入 `--rust-binary` 时会强制走对应 Rust parser CLI。
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `source_format` | string | `aedb`、`auroradb`、`odbpp`、`brd` 或 `alg`。 |
...
- Current project version: `1.0.43`
- Current Semantic parser version: `0.7.5`
- Current Semantic JSON schema version: `0.7.1`
...
For ODB++ and ALG source paths, the semantic CLI prefers the corresponding PyO3 native module by default; passing `--rust-binary` forces the corresponding Rust parser CLI.
...
| Field | Type | Description |
| --- | --- | --- |
| `source_format` | string | `aedb`, `auroradb`, `odbpp`, `brd`, or `alg`. |
